...
func TestCreateRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{
			responderURL: "http://respo.nd/er",
			expectedURL:  "http://respo.nd/er/" + ocspRequestBase64,
		},
		{
			responderURL: "https://v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/" +
				"l.o.n.g/r.e.s.p.o.n.d.e.r/u.r.l/t.o/t.r.i.g.g.e.r/p.o.s.t/r.e.q.u.e.s.t",
			expectedURL: "https://v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/v.e.r.y/" +
				"l.o.n.g/r.e.s.p.o.n.d.e.r/u.r.l/t.o/t.r.i.g.g.e.r/p.o.s.t/r.e.q.u.e.s.t",
			expectedBody: expectedBytes,
		},
	}

	for _, test := range tests {
		request, err := CreateRequest(cert, issuer, test.responderURL)
		if err != nil {
			t.Error(err)
			continue
		}

		if request.url != test.expectedURL {
			t.Errorf("request.url: got %s, want %s", request.url, test.expectedURL)
		}

		if !bytes.Equal(request.body, test.expectedBody) {
			t.Errorf("request.body: got %x, want %x", request.body, test.expectedBody)
		}

		if !request.notAfter.Equal(cert.NotAfter) {
			t.Errorf("request.notAfter: got %v, want %v", request.notAfter, cert.NotAfter)
		}

		if !reflect.DeepEqual(request.issuer, issuer) {
			t.Errorf("request.issuer: got %v, want %v", request.issuer, issuer)
		}
	}
}

func TestCreateHTTPRequest(t *testing.T) {
	ocspRequest, _ := hex.DecodeString(ocspRequestHex)
	tests := []struct {
		request        *Request
		etag           string
		...
		{
			request: &Request{
				url: "http://respo.nd/er/" + ocspRequestBase64,
			},
			etag:           `"etag string"`,
			expectedMethod: "GET",
		},
	...
	for _, test := range tests {
		request, err := test.request.createHTTPRequest(test.etag, test.lastModified)
		if err != nil {
			t.Error(err)
			continue
		}
		...
		if request.Method != test.expectedMethod {
			t.Errorf("request.Method: got %s, want %s", request.Method, test.expectedMethod)
		}
		if inm := request.Header.Get("If-None-Match"); inm != test.etag {
			t.Errorf("If-None-Match: got %s, want %s", inm, test.etag)
		}
		if ims := request.Header.Get("If-Modified-Since"); (ims == "") != test.lastModified.IsZero() {
			lm := ""
			if !test.lastModified.IsZero() {
				lm = test.lastModified.Format(http.TimeFormat)
			}
			t.Errorf("If-None-Match: got %s, want %s", ims, lm)
		}
		if request.ContentLength != 0 && request.ContentLength != int64(len(test.request.body)) {
			t.Errorf("request.ContentLength: got %d, want %d", request.ContentLength, len(test.request.body))
		}
		if request.Body != nil {
			if body, err := ioutil.ReadAll(request.Body); err != nil {
				t.Error(err)
			} else if !bytes.Equal(body, test.request.body) {
				t.Errorf("request.Body: got %x, want %x", body, test.request.body)
			}
			if err := request.Body.Close(); err != nil {
				t.Error(err)
			}
		} else if test.request.body != nil {
			t.Errorf("request.Body: got nil, want %x", test.request.body)
		}
	}
}

func TestMaxAge(t *testing.T) {
	now := time.Date(2016, 1, 10, 22, 44, 0, 0, time.UTC)
	...
	tests := []struct {
		...
		{
			input: http.Header{"Date": {"Sun, 10 Jan 2016 22:45:00 GMT"}},
		},
		{
			input:    http.Header{"Cache-Control": {"max-age=86000,public,no-transform,must-revalidate"}},
			expected: now.Add(86000 * time.Second),
		},
		{
			input: http.Header{
				"Date":          {"Sun, 10 Jan 2016 22:45:00 GMT"},
				"Cache-Control": {"max-age=86000,public,no-transform,must-revalidate"},
			},
			expected: time.Date(2016, 1, 11, 22, 38, 20, 0, time.UTC),
		},
		{
			input: http.Header{
				"Date":          {"invalid value"},
		...
		{
			input:    http.Header{"Cache-Control": {"max-age=0,public,no-transform,must-revalidate"}},
			expected: now,
		},
		{
			input: http.Header{"Cache-Control": {"max-age=-1,public,no-transform,must-revalidate"}},
		},
		{
			input:    http.Header{"Cache-Control": {"max-age=86000", "public", "no-transform", "must-revalidate"}},
			expected: now.Add(86000 * time.Second),
		...
		{
			input:    http.Header{"Cache-Control": {" max-age =\t86000 , public\t,\r\nno-transform\n, must-revalidate\t"}},
			expected: now.Add(86000 * time.Second),
		},
		{
			input:    http.Header{"Cache-Control": {"public,no-transform,max-age=86000,must-revalidate"}},
			expected: now.Add(86000 * time.Second),
		},
		{
			input: http.Header{"Cache-Control": {"public,no-transform,must-revalidate"}},
		},
		{
			input: http.Header{"Cache-Control": {""}},
		},
		{
			input:    http.Header{"Cache-Control": {`max-age=86000,foo="unterminated\",quoted-string`}},
			expected: now.Add(86000 * time.Second),
		},
		{
			input: http.Header{"Cache-Control": {`foo="unterminated\",quoted-string,max-age=86000`}},
		},
		{
			input: http.Header{"Cache-Control": {`foo="quoted\",max-age=\"86000\", must not be found"`}},
		},
		{
			input:    http.Header{"Expires": {"Mon, 11 Jan 2016 22:44:00 GMT"}},
			expected: time.Date(2016, 1, 11, 22, 44, 0, 0, time.UTC),
		},
		{
			input: http.Header{"Expires": {"invalid value"}},
		},
		{
			input:    http.Header{"Cache-Control": {""}, "Expires": {"Mon, 11 Jan 2016 22:44:00 GMT"}},
			expected: time.Date(2016, 1, 11, 22, 44, 0, 0, time.UTC),
		},
		{
			input:    http.Header{"Cache-Control": {"max-age=-1"}, "Expires": {"Mon, 11 Jan 2016 22:44:00 GMT"}},
			expected: time.Date(2016, 1, 11, 22, 44, 0, 0, time.UTC),
		},
		{
			input:    http.Header{"Cache-Control": {"max-age=86000"}, "Expires": {"Mon, 11 Jan 2016 22:44:00 GMT"}},
			expected: time.Date(2016, 1, 11, 22, 37, 20, 0, time.UTC),
		},
		{
			input:    http.Header{"Cache-Control": {"max-age=86000"}, "Expires": {"invalid value"}},
			expected: now.Add(86000 * time.Second),
		},
	}
	for _, test := range tests {
		m := maxAge(test.input, now)
		if !m.Equal(test.expected) {
			t.Errorf("maxAge(%v): got %v, want %v", test.input, m, test.expected)
		}
	}
}

func TestParseResponse(t *testing.T) {
	ocspResponse, _ := hex.DecodeString(ocspResponseHex)
	parsedOCSPResponse, err := ocsp.ParseResponse(ocspResponse, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		},
		{
			...
			expectedErr: errBadHTTPStatus(404),
		},
		{
			input: http.Response{
				StatusCode:    http.StatusOK,
				Header:        http.Header{},
				ContentLength: int64(len(ocspResponse)),
				Body:          ioutil.NopCloser(bytes.NewReader(ocspResponse)),
			},
			expectedErr: errNoContentType,
		},
		{
			input: http.Response{
				StatusCode:    http.StatusOK,
			...
			expectedErr: errBadContentType("application/octet-stream"),
		},
		{
			input: http.Response{
				StatusCode:    http.StatusOK,
				Header:        http.Header{"Content-Type": {"application/ocsp-response;key=value"}},
				ContentLength: int64(len(ocspResponse)),
				Body:          ioutil.NopCloser(bytes.NewReader(ocspResponse)),
			},
			expectedErr: errBadContentType("application/ocsp-response;key=value"),
		},
		{
			input: http.Response{
				StatusCode:    http.StatusOK,
				Header:        http.Header{"Content-Type": {"application/ocsp-response"}},
				ContentLength: int64(len(ocspResponse)),
				Body:          ioutil.NopCloser(iotest.TimeoutReader(bytes.NewReader(ocspResponse))),
			},
		...
		},
		{
			input: http.Response{
				StatusCode:    http.StatusOK,
				Header:        http.Header{"Content-Type": {"application/ocsp-response"}},
				ContentLength: int64(len(ocspResponse) / 2),
				Body:          ioutil.NopCloser(io.LimitReader(bytes.NewReader(ocspResponse), int64(len(ocspResponse)/2))),
			},
			expectedErr: asn1.SyntaxError{Msg: "data truncated"},
		},
		{
			input: http.Response{
				StatusCode:    http.StatusOK,
			...
			expected: &Response{
				OCSPResponse:    parsedOCSPResponse,
				RawOCSPResponse: ocspResponse[:],
			},
		},
		{
			input: http.Response{
				...
				Header: http.Header{
					...
					"Cache-Control": {"max-age=86000,public,no-transform,must-revalidate"},
					"Expires":       {parsedOCSPResponse.NextUpdate.Format(http.TimeFormat)},
					"Etag":          {`"the etag"`},
					"Last-Modified": {parsedOCSPResponse.ThisUpdate.Format(http.TimeFormat)},
				},
				ContentLength: int64(len(ocspResponse)),
				Body:          ioutil.NopCloser(bytes.NewReader(ocspResponse)),
			},
			expected: &Response{
				OCSPResponse:    parsedOCSPResponse,
				RawOCSPResponse: ocspResponse[:],
				MaxAge:          time.Date(2016, 1, 11, 22, 37, 20, 0, time.UTC),
				Etag:            `"the etag"`,
				LastModified:    parsedOCSPResponse.ThisUpdate,
			},
		},
	}

	for _, test := range tests {
		r, err := parseResponse(&test.input, issuer, now)
		if err != nil {
			if test.expectedErr == nil {
				t.Error(err)
			} else if !reflect.DeepEqual(err, test.expectedErr) {
				t.Errorf("parseResponse: error: got %v, want %v", err, test.expectedErr)
			}
			continue
		}
		if !reflect.DeepEqual(r, test.expected) {
			t.Errorf("parseResponse: got %v, want %v", r, test.expected)
		}
	}
}
...
func TestFetch(t *testing.T) {
	...
	tests := []struct {
		...
		action          func(int, *http.Request) (*http.Response, error)
		expectedErr     error
		expected        *Response
		expectSecondReq bool
	}{
		{
			now: parsedOCSPResponse.ThisUpdate.Add(parsedOCSPResponse.NextUpdate.Sub(parsedOCSPResponse.ThisUpdate) / 2),
			expected: &Response{
				OCSPResponse:    parsedOCSPResponse,
				RawOCSPResponse: ocspResponse,
			},
			action: func(n int, req *http.Request) (*http.Response, error) {
				resp := &http.Response{
					StatusCode:    http.StatusOK,
					Header:        http.Header{"Content-Type": {"application/ocsp-response"}},
					ContentLength: int64(len(ocspResponse)),
		...
		},
		{
			...
			expected: &Response{
				...
				Etag:            `"the etag"`,
				LastModified:    parsedOCSPResponse.ThisUpdate,
				MaxAge:          parsedOCSPResponse.NextUpdate,
			},
			action: func(n int, req *http.Request) (*http.Response, error) {
				resp := &http.Response{
					StatusCode: http.StatusOK,
		...
		},
		{
			...
			etag:     `"the etag"`,
			action: func(n int, req *http.Request) (*http.Response, error) {
				if inm := req.Header.Get("If-None-Match"); inm != `"the etag"` {
					t.Errorf(`fetcher: If-None-Match: got %s, want "the etag"`, inm)
				}
				resp := &http.Response{
					StatusCode:    http.StatusNotModified,
					ContentLength: 0,
		...
		},
		{
			...
			lastModified: parsedOCSPResponse.ThisUpdate,
			action: func(n int, req *http.Request) (*http.Response, error) {
				if ims, err := http.ParseTime(req.Header.Get("If-Modified-Since")); err != nil {
					t.Error(err)
				} else if !parsedOCSPResponse.ThisUpdate.Truncate(time.Second).Equal(ims) {
					t.Errorf("fetcher: If-Modified-Since: got %v, want %v", ims, parsedOCSPResponse.ThisUpdate)
				}
				resp := &http.Response{
					StatusCode:    http.StatusNotModified,
					ContentLength: 0,
		...
		{
			now:         cert.NotAfter.Add(1 * time.Hour),
			expectedErr: errCertExpired,
			action: func(n int, req *http.Request) (*http.Response, error) {
		...
		{
			requests: []*Request{getRequest},
			now:      parsedOCSPResponse.NextUpdate.Add(1 * time.Hour),
			expected: &Response{
				OCSPResponse:    parsedOCSPResponse,
				RawOCSPResponse: ocspResponse,
			},
			expectSecondReq: true,
			action: func(n int, req *http.Request) (*http.Response, error) {
				if n > 1 {
					if cc := req.Header.Get("Cache-Control"); cc != "no-cache" {
						t.Errorf("fetcher: second request missing Cache-Control:no-cache (got %s)", cc)
					}
				}
				resp := &http.Response{
					StatusCode:    http.StatusOK,
		...
		},
		{
			requests:        []*Request{getRequest},
			now:             parsedOCSPResponse.NextUpdate.Add(1 * time.Hour),
			expectedErr:     errBadHTTPStatus(500),
			expectSecondReq: true,
			action: func(n int, req *http.Request) (*http.Response, error) {
				switch n {
				case 1:
		...
		{
			requests: []*Request{getRequest},
			now:      parsedOCSPResponse.ThisUpdate.AddDate(0, 0, 1),
			expected: &Response{
				OCSPResponse:    parsedOCSPResponse,
				RawOCSPResponse: ocspResponse,
			},
			expectSecondReq: true,
			nextUpdate:      parsedOCSPResponse.ThisUpdate,
			action: func(n int, req *http.Request) (*http.Response, error) {
				switch n {
				case 1:
					resp := &http.Response{
	...
	for _, test := range tests {
		reqs := test.requests
		if len(reqs) == 0 {
			reqs = []*Request{getRequest, postRequest}
		}
		for _, req := range reqs {
			...
			f := Fetcher{
				...
				time: func() time.Time { return test.now },
			}
			resp, err := f.Fetch(req, test.etag, test.lastModified, test.nextUpdate)
			if err != nil {
				if test.expectedErr == nil {
					t.Error(err)
				} else if !reflect.DeepEqual(err, test.expectedErr) {
					t.Errorf("fetcher.Fetch: error: got %v, want %v", err, test.expectedErr)
				}
				continue
			}

			expectedReqs := 1
			if test.expectSecondReq {
				expectedReqs = 2
			}
			if crt.n != expectedReqs {
				t.Errorf("fetcher.Fetch: request count: got %d, want %d", crt.n, expectedReqs)
			}

			if !reflect.DeepEqual(resp, test.expected) {
				t.Errorf("fetcher.Fetch: got %v, want %v", resp, test.expected)
			}
		}
	}
}

// ocspRequestBase64 is ocspRequestHex decoded to bytes, then encoded to Base64, and finally URL-encoded.
// Note that it contains all Base64 non-URL-safe characters +, / and =
const ocspRequestBase64 = "MFEwTzBNMEswSTAJBgUrDgMCGgUABBTA%2FgJ4%2FJkYiJGz8hLpx%2BGyGre%2FwAQUDfwd" +
	"8Kng8Bzn8rITF35vjRV81PYCEAF%2Fd96zvLsjXUTMx9umLnI%3D"

// All following constants were copied from golang.org/x/crypto/ocsp/ocsp_test.go.
// Copyright 2013 The Go Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be found at
// https://go.googlesource.com/crypto/+/f18420efc3b4f8e9f3d51f6bd2476e92c46260e9/LICENSE.
